...
<Accordion title="Which models are in each tier?">
  **Small (S)** — Example: `llama-3.2-3b`

  **Medium (M)** — Example: `llama-3.3-70b` `qwen3-next-80b` `deepseek-v4-flash`

  **Large (L)** — Example: `glm-5` `glm-5.1` `kimi-k2.5` `kimi-k2.6` `MiniMax-M2.7` `deepseek-v4-pro`
...
If your workload requires high throughput, consider using a **Small** tier model like `llama-3.2-3b` which allows up to 500 requests/min. Reserve **Large** tier models for tasks that genuinely require their additional capability.
</Accordion>

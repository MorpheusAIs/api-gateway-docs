...
```bash cURL
curl --request POST \
  --url 'https://api.dev.mor.org/api/v1/session/modelsession?model_id=0x1234567890abcdef' \
...
const response = await fetch(
  'https://api.dev.mor.org/api/v1/session/modelsession?model_id=0x1234567890abcdef',
  {
    method: 'POST',
    headers: {
...
response = requests.post(
    'https://api.dev.mor.org/api/v1/session/modelsession',

...
```bash cURL
curl --request POST \
  --url 'https://api.mor.org/api/v1/session/bidsession?bid_id=0x1234567890abcdef' \
...
const response = await fetch(
  'https://api.mor.org/api/v1/session/bidsession?bid_id=0x1234567890abcdef',
  {
    method: 'POST',
    headers: {
...
response = requests.post(
    'https://api.mor.org/api/v1/session/bidsession',

...
```bash cURL
curl --request GET \
  --url 'https://api.mor.org/api/v1/models/ratedbids?model_id=0x1234567890abcdef' \
  --header 'Authorization: Bearer sk-YOUR_API_KEY'
```

```javascript JavaScript
const response = await fetch(
  'https://api.mor.org/api/v1/models/ratedbids?model_id=0x1234567890abcdef',
  {
    method: 'GET',
    headers: {
...
response = requests.get(
    'https://api.mor.org/api/v1/models/ratedbids',
